...
if( isset($_GET['order']) && isset($_GET['column']) ) {
    ...
    elseif( $_GET['colum'] = 'firstname' ){$oder = ' ORDER BY firstname';
    }
...
if(!empty($_POST)) {
    ...
    if(strlen($_POST['firstname']) < 3) { $errors[] = 'Le prénom doit comporter au moins 3 caractères';
    }
    if(strlen($_POST['lastname']) < 3) { $errors[] = 'Le nom doit comporter au moins 3 caractères';
    }
    if(!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) { $errors[] = 'L\'adresse email est invalide';
    }
    if(empty($_POST['birthdate'])) { $errors[] = 'La date de naissance doit être complétée';
    }
    if(empty($_POST['city'])){ $errors[] = 'La ville ne peut être vide';
    }
    if(count($errors) == 0) {
        // erreurr = 0 = insertion user
        $insertUser = $db->prepare
        ('INSERT INTO users (gender, firstname, lastname, email, birthdate, city) 
